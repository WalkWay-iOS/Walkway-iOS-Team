...
extension ProfileKeywordTVC {
    ...
    func setButton() {
        ...
        keywordEditButton.setTitleColor(.systemIndigo, for: .normal)
        keywordEditButton.addTarget(self, action: #selector(touchUpEditKeyword), for: .touchUpInside)
    }
}

// MARK: - Action
extension ProfileKeywordTVC {
    @objc func touchUpEditKeyword() {
        delegate?.cellTapedEditKeyword()
    }
}
