...
extension BannerVC {
    ...
    private func setSeoul() {
        country.append(contentsOf: [
            Country(title: "서울 둘레길", courses: 14, content: "서울을 한 바퀴 휘감는 총 연장 157km의 서울둘레길은 8개 코스로 서울의 역사, 문화, 자연생태 등을 스토리로 엮어 국내외 탐방객들이 느끼고, 배우고, 체험할 수 있도록 조성한 도보길입니다. 서울둘레길은 ‘숲길’, ‘하천길’, ‘마을길’로 구성되어 있습니다. 둘레길 곳곳에 휴게시설과 북카페, 쉼터를 만들어 시민들이 자연스럽게 휴식을 취할 수 있게 했고, 전통 깊은 사찰과 유적지을 연결해 서울의 역사와 문화, 자연생태를 곳곳에서 체험할 수 있도록 조성하였습니다. 대중교통으로도 접근하기 쉬우며 주로 경사가 심하지 않은 흙길로 되어 있어 누구나 안전하고 편안하게 이용할 수 있습니다.")
        ])
        setSeoulBanner()
    }
...
extension BannerVC {
    func setSeoulBanner() {
        authProvider.request(.seoulCourses) { response in
            switch response {
    ...
    func setJejuBanner() {
        authProvider.request(.jejuCourses) { response in
            switch response {
            case .success(let result):
                do {
                    self.homeModel = try result.map(ViewAllModel.self)
